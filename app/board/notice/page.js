...
export default function Home() {
  ...
  async function setUp() {
    await fetch('/api/board/prevRead/?type=notice&start=0&cnt=10')
        .then((res) => res.json())
        .then(async (res) => {
  ...
  async function onMore() {
    setLoad(true)
    await fetch(`/api/board/prevRead/?type=notice&start=${postCnt}&cnt=10`)
        .then((res) => res.json())
        .then(async (res) => {
            await res.data.forEach(element => {
                setPost(prevList => [...prevList, element])
            ...
            <div className="boardMainCxt">
                ...
                <div className="boardPop">
                    <div className="boardttop">
                        <h2>공지 게시판</h2>
                        {   
                            (session?.user.email == 'imaecommunity@gmail.com') ? (
                                ...
                                return (<div className="postPrevCxt" onClick={() => {window.location.href=`/board/read/${element.id}`}}>
                                    <div className="txtCxt">
                                        <div className="TitleCxt">
                                            <div className="boardType">공지</div>
